...
@interface RCTImagePickerModule ()
...
RCT_EXPORT_METHOD(launchCamera:(NSDictionary *)options
...
RCT_EXPORT_METHOD(launchImageLibrary:(NSDictionary *)options
                  callback:(RCTResponseSenderBlock)callback)
{
    self.callback = callback;
        ...
        if ([mediaType isEqualToString:(NSString *)kUTTypeImage]) { // PHOTOS
            ...
            if (imageURL) {
                ...
                if (pickedAsset.creationDate) {
                    self.response[@"timestamp"] = [[[self class] ISO8601DateFormatter] stringFromDate:pickedAsset.creationDate];
                }
            }
                        ...
                        if (error) {
                            ...
                            if (assetURL) {
                                ...
                                if (capturedAsset.creationDate) {
                                    self.response[@"timestamp"] = [[[self class] ISO8601DateFormatter] stringFromDate:capturedAsset.creationDate];
                                }
                            }
