...
public class VideoNoticeView extends RelativeLayout implements View.OnTouchListener{
    ...
    private void initView() {
        LayoutInflater.from(getContext()).inflate(com.lecloud.skin.R.layout.letv_skin_v4_notice_layout, this, true);
        mErrorCode = (TextView) findViewById(com.lecloud.skin.R.id.tv_error_code);
        mErrorReason = (TextView) findViewById(com.lecloud.skin.R.id.tv_error_message);
        mTips = (TextView) findViewById(com.lecloud.skin.R.id.tv_error_msg);
        mReplay = (Button) findViewById(com.lecloud.skin.R.id.btn_error_replay);
        mErrorReport = (Button) findViewById(com.lecloud.skin.R.id.btn_error_report);
...
//        });
    ...
    private void showMsg(String event, String msg) {
        if (event == null || event.equals("") || event.equals("0") || event.equals("-1")) {
            ...
            mErrorCode.setText(getContext().getResources().getString(com.lecloud.skin.R.string.error_code) + event);
        }
...
//            mEventCode = data.getString(PlayerParams.KEY_RESULT_ERROR_CODE, "");
            if (status == StatusCode.MEDIADATA_NETWORK_ERROR) {
                String errMsg = getText(com.lecloud.skin.R.string.request_fail);
                videoTips(mEventCode, errMsg, getText(com.lecloud.skin.R.string.letv_notice_message), getText(com.lecloud.skin.R.string.replay), getText(com.lecloud.skin.R.string.submit_error_info));
            } else if (status == StatusCode.MEDIADATA_INTERNAL_ERROR) {
                String errMsg = getText(com.lecloud.skin.R.string.play_error);
                videoTips(mEventCode, errMsg, getText(com.lecloud.skin.R.string.letv_notice_message), getText(com.lecloud.skin.R.string.replay), getText(com.lecloud.skin.R.string.submit_error_info));
            ...
            videoTips("" + 10000, getText(com.lecloud.skin.R.string.net_fail), getText(com.lecloud.skin.R.string.net_error), getText(com.lecloud.skin.R.string.replay), getText(com.lecloud.skin.R.string.submit_error_info));
        }
    }
    ...
    public void processActionStatus(int status) {
        ...
        if (status == ActionStatus.SATUS_NOT_START) {
            errMsg = getText(com.lecloud.skin.R.string.live_no_start);
        } else if (status == ActionStatus.STATUS_END) {
            errMsg = getText(com.lecloud.skin.R.string.live_end);
        } else if (status == ActionStatus.STATUS_LIVE_ING) {
            errMsg = getText(com.lecloud.skin.R.string.live_sig_recovery);
        } else {
            errMsg = getText(com.lecloud.skin.R.string.live_no_sig);
        }
        videoTips("", "", errMsg, getText(com.lecloud.skin.R.string.replay), getText(com.lecloud.skin.R.string.submit_error_info));
    }
    ...
    private void processActionMediaError(Bundle data) {
        ...
        if ("E06101".equals(mEventCode)) {
            // 超出观看人数
            errMsg = getText(com.lecloud.skin.R.string.people_more);
        } else if ("E06102".equals(mEventCode)) {
            // 代理服务器出现黑名单，无法播放
            errMsg = getText(com.lecloud.skin.R.string.proxy_black_list);
        } else if ("E06103".equals(mEventCode)) {
            // 加密失败
            errMsg = getText(com.lecloud.skin.R.string.linkshell_fail);
        } else if ("E06104".equals(mEventCode)) {
            // 无直播计划
            errMsg = getText(com.lecloud.skin.R.string.no_live_plan);
        } else {
            // 未知错误
...
//                mEventCode = "" + event;
                if(code == StatusCode.PLAY_ERROR_NONETWORK){
                	...
                	videoTips("" + code, getText(com.lecloud.skin.R.string.play_fail), getText(com.lecloud.skin.R.string.letv_notice_message), getText(com.lecloud.skin.R.string.replay), getText(com.lecloud.skin.R.string.submit_error_info));
                }
                break;
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		if(event.getAction() == MotionEvent.ACTION_UP){
			...
			if(v.getId() == com.lecloud.skin.R.id.btn_error_replay){
				v.performClick();
			     // 重新播放
                if (!NetworkUtils.hasConnect(mContext)) {
                    Toast.makeText(mContext, getText(com.lecloud.skin.R.string.network_none), Toast.LENGTH_SHORT).show();
                    return true;
                }
                setVisibility(View.GONE);
                mRePlayListener.onRePlay();
			}else if(v.getId() == com.lecloud.skin.R.id.btn_error_report){
				v.performClick();
				  // 错误反馈
                Intent intent = new Intent(mContext, FeedBackActivity.class);
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
