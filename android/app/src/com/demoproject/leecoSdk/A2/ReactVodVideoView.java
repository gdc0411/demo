...
public class ReactVodVideoView extends VodVideoView {
    ...
    */
    ...
    public ReactVodVideoView(Context context) {
        ...
        //设置声音管理器
        mAudioManager = (AudioManager) mThemedReactContext.getSystemService(Service.AUDIO_SERVICE);
...
//        mNoticeView.processPlayerState(event, bundle);
    ...
    public void onPause() {
        super.onPause();
        if (player != null) {
            Log.d(TAG, LogUtils.getTraceInfo() + "生命周期事件 onPause 调起！");
            saveLastPostion();
            player.pause();
        }
    }
    ...
    public void onResume() {
        if (player != null) {
            Log.d(TAG, LogUtils.getTraceInfo() + "生命周期事件 onResume 调起！");
            player.retry();
        }
        super.onResume();
    }
    ...
    public void onDestroy() {
        super.onDestroy();
        if (player != null) {
            Log.d(TAG, LogUtils.getTraceInfo() + "生命周期事件 onDestroy 调起！");
            player.release();
